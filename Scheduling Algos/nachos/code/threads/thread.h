...
class NachOSThread {
  ...
  public:
    ...
    ~NachOSThread(); 				// deallocate a Thread
					...
					// is called
        int cpu_count,priority,base_priority;
    ...
    int wait_start;				//Used to compute wait time of the process
    int thread_burst,estimated_burst;           //Used for SJF 
    void CheckOverflow();   			// Check if thread has 
						// overflowed its stack
    void setStatus(ThreadStatus st) { status = st; }
    inline ThreadStatus getStatus(void){return status;} //Used to getstatus of the thread
