...
NachOSThread::NachOSThread(char* threadName)
{
    ...
    instructionCount = 0;
    completion_time[pid]=stats->totalTicks;       //Start execution time of the process
    thread_burst = 0;				  //Used to store current burst of the thread
    estimated_burst = TimerTicks;                 //Initial guess for SJF
}
...
{
    current_burst = stats->totalTicks - current_burst;
    thread_burst = current_burst;
    if(current_burst > 0){
        number_bursts++;

        if(max_burst < current_burst) // for calculation of cpu burst statement 
            max_burst = current_burst;
        if(min_burst > current_burst)
            min_burst = current_burst;
        total_burst = total_burst + current_burst;
        //printf("thread with pid %d    %d : %d\n",currentThread->GetPID(),stats->totalTicks,currentThread->cpu_count);
	if (s_algo==2){
		...
		currentThread->estimated_burst = (currentThread->thread_burst + currentThread->estimated_burst)/2;
	}
        if (s_algo>6){
    ...
    DEBUG('t', "Finishing thread \"%s\" with pid %d\n", getName(), pid);
    completion_time[pid]=stats->totalTicks;     //Marks completion time of a thread 
//	printf("Finishing thread %d : time %d\n",pid,completion_time[pid]);    
   threadToBeDestroyed = currentThread;
    ...
    while ((nextThread = scheduler->SelectNextReadyThread()) == NULL) {
        if (terminateSim) {
           ...
           //printf("Main thread time : %d\n",mte);
	   printf("\nNo threads ready or runnable, and no pending interrupts.\n");
           printf("Assuming all programs completed.\n");
	   printf("Total CPU busy time = %d\n",total_burst);
	   printf("Total execution Time = %d\n",stats->totalTicks-mte);   //mte = main thread execution time
           printf("CPU utilisation : %.2f%\n",(1.0*total_burst/(stats->totalTicks-mte))*100);
	   printf("Average CPU burst : %f\n",1.0*total_burst/number_bursts);
	   printf("Maximum CPU burst : %d\n",max_burst);
	   printf("Minimum CPU burst : %d\n",min_burst);
	   printf("Total number of CPU bursts : %d\n",number_bursts);
	   printf("Average Waiting Time : %f\n", 1.0*total_wait/(thread_index-1));
	   long long int total_completion=0,min_completion=completion_time[thread_index-1],max_completion=completion_time[thread_index-1];
	   for(int i=1;i<thread_index;i++){ 
	//	printf("%d : Time %d\n",i,completion_time[i]);
		total_completion+=completion_time[i]; 
		if(completion_time[i]<min_completion){ min_completion = completion_time[i]; }
		if(completion_time[i]>max_completion) max_completion = completion_time[i];
		}
	   float ssq=0;
	   float avg_completion=1.0*total_completion/(thread_index-1);
           for(int i=1;i<thread_index;i++){ssq+=(completion_time[i]-avg_completion)*(completion_time[i]-avg_completion);}
	   printf("Total completion time = %lld\n",total_completion);
	   printf("Max completion time = %lld\n",max_completion);
	   printf("Min completion time = %lld\n",min_completion);
	   printf("Average completion time = %f\n",avg_completion);
	   printf("Variance of completion time = %f\n",ssq);
	   if (s_algo==2){
		printf("Error in estimating bursts : %f\n",1.0*error/total_burst); }
	   interrupt->Halt();
        }
...
NachOSThread::YieldCPU ()
{
    ...
    if(current_burst > 0){
        ...
        total_burst = total_burst + current_burst;
       // printf("****************************************************************************");
       // printf("%dth    %d : %d\n",scheduler->number_bursts,stats->totalTicks,scheduler->current_burst);
//        printf("thread with pid %d    %d : %d\n",currentThread->GetPID(),stats->totalTicks,currentThread->cpu_count);
	if (s_algo==2){
                error += abs(currentThread->estimated_burst-current_burst); 
		//printf("Error : pid  %d : %d\n",currentThread->GetPID(),currentThread->estimated_burst-current_burst);
...
NachOSThread::PutThreadToSleep ()
{
    ...
    if(current_burst > 0){
        number_bursts++;
	//printf("   %d number of bursts %d \n",scheduler->number_bursts,currentThread->cpu_count);
        if(max_burst < current_burst) // for calculation of cpu burst statement 
            max_burst = current_burst;
        if(min_burst > current_burst)
            min_burst = current_burst;
